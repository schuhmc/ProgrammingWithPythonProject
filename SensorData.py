...
class SensorData:
    ...
    def fetch(self, last_n_days, time_column="time", col_names=[], col_units=[]):
        ...
        if self.connection_type == "web":
            # Fetch data from web
            request_url = (
                self.url + "?apikey=" + self.apikey + "&days=" + str(last_n_days)
            )
            ...
            query = "SELECT * FROM {table} WHERE {time_field} > DATE_SUB(now(), INTERVAL {days} DAY);".format(
                table=self.sql_credentials[4],
                time_field=self.sql_credentials[5],
    ...
    def summary(self, columns=[]):
        ...
        for col in columns:
            ...
            summary = summary.format(
                colname=col.lower(),
                current_value=self.df.iloc[-1][col],
                current_time=self.df.iloc[-1][self.df.columns[0]],
                unit=self.units[col],
                n_days=self.last_n_days,
                min_value=self.df.at[min_index, col],
                min_time=self.df.at[min_index, self.df.columns[0]],
                max_value=self.df.at[max_index, col],
                max_time=self.df.at[max_index, self.df.columns[0]],
            )
    ...
    def status(
        ...
        battery_column="Battery",
        battery_low_level=3.2,
        update_interval_warning_factor=2,
    ):
        """
        ...
        if time_diff.seconds > update_interval_s * update_interval_warning_factor:
            statusmsg += "<em style='color:green;'>WARNING:</em> I have not recieved data since {last_update}, which was {min_ago:.1f} minutes before this report was generated!<br>".format(
                last_update=self.df.iloc[-1]["Time"], min_ago=time_diff.seconds / 60
            )
            warning = True

        if self.df[battery_column].iloc[-1] < battery_low_level:
            statusmsg += (
                "<em style='color:red;'>WARNING:</em> Battery level is low!<br>"
            )
            warning = True

        if not warning:
            statusmsg = (
                "<em style='color:green;'>Everything seems to be OK</em><br>"
                + statusmsg
            )

        statusmsg = (
            "Data fetched on {fetch_timestamp}<br>Last update revieved on {last_update} ({min_ago:.1f} minutes before this report)<br>".format(
                fetch_timestamp=self.fetch_timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                last_update=self.df.iloc[-1]["Time"],
                min_ago=time_diff.seconds / 60,
            )
            + statusmsg
            + "<br><br>"
        )
    ...
    def simplePlot(self, column, ax="", title="", color="black"):
        ...
        if ax != "":
            ...
            sns.lineplot(
                ax=ax, data=self.df, x=self.df.columns[0], y=column, color=color
            )
    ...
    def dailyMinMaxPlot(self, column, ax="", title="", colors=["blue", "red"]):
        ...
        if ax != "":
            ...
            sns.lineplot(
                x=max_values.index, y=max_values.values, color=colors[1], marker="x"
            )
            sns.lineplot(
                x=min_values.index, y=min_values.values, color=colors[0], marker="x"
            )
    ...
    def heatmap(self, columns, ax="", title="Heatmap of Standard Scores", labels=[]):
        ...
        if ax != "":
            ax.set(title=title)
            sns.heatmap(
                hourly_mean_norm.transpose(),
                yticklabels=labels,
                ax=ax,
                cbar_kws={"label": "Standard Score"},
                center=0,
                cmap="BuPu",
            )
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Program that generates nice html output from weather data.\nNote: csv method will always use the whole csv file and the displayed n last days are not correct!"
    )
    parser.add_argument(
        "-m",
        "--method",
        help="Set the connection type 'web', 'csv' or 'sql'. ",
        required=True,
        type=str,
    )
    parser.add_argument(
        ...
        required=False,
        default="./weather_template.html",
        type=str,
    )
    parser.add_argument(
        "-o",
        "--outfile",
        help="Output file to write the generated html to.",
        required=False,
        type=str,
    ...
    parser.add_argument(
        "-p",
        "--plotfile",
        help="File the plot should be written to.",
        required=False,
        type=str,
        default="./plots.svg",
    )
    parser.add_argument(
        "-d",
        "--days",
        help="Number of last n days to fetch data from. Default is 14",
        required=False,
        default=14,
        type=int,
    )
    parser.add_argument(
        "-c",
        "--credentials",
        help=".conf file containing credentials for web and sql method as well as column names and units. See example.conf for more information",
        required=True,
        type=str,
    )
    parser.add_argument(
        "-f",
        "--csvfile",
        help="Path to CSV file if using method 'csv'",
        required=False,
        type=str,
    )
    ...
    if args.method == "web":
        ...
        con = SensorData(
            "web", url=config["web"]["url"], apikey=config["web"]["apikey"]
        )
    ...
    con.simplePlot(
        column="Temperature", title="Temeprature", ax=axes[0, 0], color="white"
    )
    ...
    con.dailyMinMaxPlot(
        column="Humidity",
        title="Daily min/max Humidity",
        ax=axes[1, 1],
        colors=["blue", "red"],
    )

    con.simplePlot(
        column="Pressure", title="Air Pressure", ax=axes[2, 0], color="white"
    )
    con.rollingAverage("Wind", "Wind rolling", 5)
    con.simplePlot(
        column="Wind rolling",
        title="Wind Speed (average over last 5 measurements)",
        ax=axes[2, 1],
        color="white",
    )
    con.simplePlot(
        column="Air Quality", title="Air Quality", ax=axes[3, 1], color="white"
    )
    con.simplePlot(
        column="Illuminance", title="Illuminance", ax=axes[3, 0], color="white"
    )
    con.dailyMinMaxPlot(
        column="Illuminance",
        title="Daily min/max Illuminance",
        ax=axes[4, 0],
